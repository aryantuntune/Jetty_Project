...
<div class="container list-window">
    <div class="list-body">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h2 class="page-title">Edit Operator</h2>
            <a href="{{ route('operator.index') }}" class="btn btn-secondary">↩ Back to List</a>
        </div>
        ...
        <form action="{{ route('operator.update', $operator) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="email" name="email" value="{{ old('email', $operator->email) }}" class="form-control" required>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="mobile" value="{{ old('mobile', $operator->mobile) }}" class="form-control">
                ...
                <select name="branch_id" class="form-select">
                    ...
                    @foreach($branches as $branch)
                        <option value="{{ $branch->id }}"
                            {{ (string) old('branch_id', $operator->branch_id) === (string) $branch->id ? 'selected' : '' }}>
                            {{ $branch->branch_name }}
                        </option>
                    @endforeach
                </select>
            ...
            {{-- Keep Ferryboat for future use (consistent with other pages)
            <div class="mb-3">
                ...
                <select name="ferryboat_id" class="form-select">
                    ...
                    @foreach($ferryboats as $ferryboat)
                        <option value="{{ $ferryboat->id }}"
                            {{ (string) old('ferryboat_id', $operator->ferryboat_id) === (string) $ferryboat->id ? 'selected' : '' }}>
                            {{ $ferryboat->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="d-flex justify-content-end gap-2">
                <a href="{{ route('operator.index') }}" class="btn btn-secondary">↩ Cancel</a>
