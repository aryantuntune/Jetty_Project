...
<div class="max-w-2xl mx-auto">
    <!-- Header -->
    <div class="mb-6">
        <a href="{{ route('checker.index') }}" class="inline-flex items-center text-slate-500 hover:text-slate-700 transition-colors mb-4">
            <i data-lucide="arrow-left" class="w-4 h-4 mr-2"></i>
            Back to Checkers
        </a>
        <h2 class="text-2xl font-bold text-slate-800">Edit Checker</h2>
        <p class="text-slate-500 mt-1">Update checker details and branch assignment</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('checker.update', $checker) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $checker->name) }}" required
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $checker->email) }}" required
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500">Leave blank if you don't want to change the password</p>
                @error('password')
                <p class="mt-2 text-sm text-red-600 flex items-center">
                    <i data-lucide="alert-circle" class="w-4 h-4 mr-1"></i>
                    {{ $message }}
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="mobile" id="mobile" value="{{ old('mobile', $checker->mobile) }}"
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <select name="branch_id" id="branch_id"
                        ...
                        <option value="{{ $branch->id }}" {{ (string) old('branch_id', $checker->branch_id) === (string) $branch->id ? 'selected' : '' }}>
                            {{ $branch->branch_name }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="flex items-center justify-end space-x-3 pt-6 border-t border-slate-200">
                <a href="{{ route('checker.index') }}" class="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-medium hover:bg-slate-50 transition-colors">
                    Cancel
                </a>
