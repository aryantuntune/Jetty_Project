...
<style>
    ...
    .form-input { border: 1px solid #ccc; padding: 6px; width: 100%; font-size: 14px; border-radius: 3px; }
    .btn-save { background: #1d4ed8; color: white; padding: 6px 12px; border-radius: 3px; border: none; font-size: 14px; }
    .btn-back { background: gray; color: white; padding: 6px 12px; border-radius: 3px; border: none; font-size: 14px; text-decoration: none; }
...
<div class="flex justify-center items-center min-h-screen bg-gray-200 container p-4">
    <div class="win-card w-1/2">
        ...
        <!-- Form -->
        <form action="{{ route('guest_categories.store') }}" method="POST" class="p-4 space-y-3">
            @csrf

            <div>
                <label for="name" class="block font-bold text-sm">Category Name</label>
                <input type="text" name="name" id="name" class="form-input" required>
            </div>

            <!-- Footer -->
            <div class="win-footer mt-4">
                <a href="{{ route('guest_categories.index') }}" class="btn-back">Back</a>
