        ...
        <aside id="sidebar" class="mobile-sidebar fixed lg:static inset-y-0 left-0 z-50 w-64 bg-sidebar flex flex-col">
            ...
            <div class="flex items-center h-16 px-6 border-b border-white/10">
                <a href="{{ route('home') }}" class="flex items-center space-x-3">
                    <div
                        class="w-9 h-9 rounded-lg bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center">
            ...
            <nav class="flex-1 px-3 py-4 sidebar-scroll overflow-y-auto">
                ...
                @if($isSuperAdmin || $isAdmin || $isManager || $isOperator)
                    <a href="{{ route('ticket-entry.create') }}"
                        class="nav-item {{ $currentRoute == 'ticket-entry.create' ? 'active' : '' }}">
                    ...
                    <div class="nav-group">
                        ...
                        <div class="submenu {{ $isReportsActive ? 'open' : '' }}">
                            <a href="{{ route('reports.tickets') }}"
                                class="submenu-item {{ $currentRoute == 'reports.tickets' ? 'active' : '' }}">Ticket
                                Details</a>
                            <a href="{{ route('reports.vehicle_tickets') }}"
                                class="submenu-item {{ $currentRoute == 'reports.vehicle_tickets' ? 'active' : '' }}">Vehicle-wise
                                Details</a>
                        </div>
                    </div>
                @endif
                ...
                @if($isSuperAdmin || $isAdmin)
                    <div class="nav-group">
                        ...
                        <div class="submenu {{ $isMastersActive ? 'open' : '' }}">
                            <a href="{{ route('items.from_rates.index') }}"
                                class="submenu-item {{ $currentRoute == 'items.from_rates.index' ? 'active' : '' }}">Items</a>
                            <a href="{{ route('item_categories.index') }}"
                                class="submenu-item {{ Str::startsWith($currentRoute, 'item_categories.') ? 'active' : '' }}">Item
                                Categories</a>
                            <a href="{{ route('item-rates.index') }}"
                                class="submenu-item {{ Str::startsWith($currentRoute, 'item-rates.') ? 'active' : '' }}">Item
                                Rate Slabs</a>
                            <a href="{{ route('ferryboats.index') }}"
                                class="submenu-item {{ Str::startsWith($currentRoute, 'ferryboats.') ? 'active' : '' }}">Ferry
                                Boats</a>
                            <a href="{{ route('ferry_schedules.index') }}"
                                class="submenu-item {{ Str::startsWith($currentRoute, 'ferry_schedules.') ? 'active' : '' }}">Ferry
                                Schedules</a>
                            <a href="{{ route('guests.index') }}"
                                class="submenu-item {{ Str::startsWith($currentRoute, 'guests.') ? 'active' : '' }}">Guests</a>
                            <a href="{{ route('guest_categories.index') }}"
                                class="submenu-item {{ Str::startsWith($currentRoute, 'guest_categories.') ? 'active' : '' }}">Guest
                                Categories</a>
                            <a href="{{ route('branches.index') }}"
                                class="submenu-item {{ Str::startsWith($currentRoute, 'branches.') ? 'active' : '' }}">Branches</a>
                            <a href="{{ route('special-charges.index') }}"
                                class="submenu-item {{ Str::startsWith($currentRoute, 'special-charges.') ? 'active' : '' }}">Special
                                Charges</a>
                        </div>
                    </div>
                @endif

                {{-- Houseboat Management - visible to Super Admin and Admin only --}}
                @if($isSuperAdmin || $isAdmin)
                    @php $isHouseboatActive = Str::startsWith($currentRoute, 'admin.houseboat.'); @endphp
                    <div class="nav-group">
                        <div class="nav-item {{ $isHouseboatActive ? 'active open' : '' }}" onclick="toggleSubmenu(this)">
                            <i data-lucide="anchor"></i>
                            <span>Houseboat</span>
                            <i data-lucide="chevron-down" class="chevron"></i>
                        </div>
                        <div class="submenu {{ $isHouseboatActive ? 'open' : '' }}">
                            <a href="{{ route('admin.houseboat.dashboard') }}"
                                class="submenu-item {{ $currentRoute == 'admin.houseboat.dashboard' ? 'active' : '' }}">Bookings</a>
                            <a href="{{ route('admin.houseboat.rooms') }}"
                                class="submenu-item {{ $currentRoute == 'admin.houseboat.rooms' ? 'active' : '' }}">Rooms</a>
                        </div>
                    </div>
                @endif
                ...
                @if($isSuperAdmin || $isAdmin)
                    <a href="{{ route('employees.transfer.index') }}"
                        class="nav-item {{ $isTransferActive ? 'active' : '' }}">
                        ...
                        <div class="submenu {{ $isAdminActive ? 'open' : '' }}">
                            ...
                            @if($isSuperAdmin)
                                <a href="{{ route('admin.index') }}"
                                    class="submenu-item {{ Str::startsWith($currentRoute, 'admin.') ? 'active' : '' }}">Administrators</a>
                            @endif
                            {{-- Only Super Admin and Admin can see/manage Managers --}}
                            @if($isSuperAdmin || $isAdmin)
                                <a href="{{ route('manager.index') }}"
                                    class="submenu-item {{ Str::startsWith($currentRoute, 'manager.') ? 'active' : '' }}">Managers</a>
                            @endif
                            {{-- Super Admin, Admin, and Manager can see Operators --}}
                            <a href="{{ route('operator.index') }}"
                                class="submenu-item {{ Str::startsWith($currentRoute, 'operator.') ? 'active' : '' }}">Operators</a>
                            {{-- Super Admin, Admin, and Manager can see Checkers --}}
                            <a href="{{ route('checker.index') }}"
                                class="submenu-item {{ Str::startsWith($currentRoute, 'checker.') ? 'active' : '' }}">Checkers</a>
                        </div>
                    </div>
                @endif
            </nav>
            ...
            <div class="p-4 border-t border-white/10">
                <div class="flex items-center space-x-3">
                    <div
                        class="w-10 h-10 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white font-semibold">
                    ...
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="p-2 rounded-lg text-white/60 hover:bg-sidebar-hover hover:text-red-400 transition-colors"
                            title="Logout">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="sticky top-0 z-30 bg-white border-b border-slate-200 shadow-sm">
                <div class="flex items-center justify-between h-16 px-4 lg:px-8">
                    ...
                    <!-- Page Title -->
                    <h1 class="text-lg font-semibold text-slate-800 hidden lg:block">@yield('page-title', 'Dashboard')
                    </h1>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="hidden md:flex items-center">
                            <div class="relative">
                                <input type="text" placeholder="Search..."
                                    class="w-64 pl-10 pr-4 py-2 rounded-lg border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none text-sm">
                        ...
                        <div class="lg:hidden">
                            <div
                                class="w-8 h-8 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white text-sm font-semibold">
                                {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                            </div>
                        </div>
                    </div>
