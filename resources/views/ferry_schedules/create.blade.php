...
@section('title', 'Add Ferry Schedule')
@section('page-title', 'Add Ferry Schedule')
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-emerald-600 to-emerald-700">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h2 class="font-semibold text-white">Add New Ferry Schedule</h2>
                    <p class="text-sm text-emerald-100">Fill in the details below</p>
                </div>
            </div>
        </div>

        <form action="{{ route('ferry_schedules.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="hour" id="hour" value="{{ old('hour') }}" min="0" max="23" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none" placeholder="0-23" required>
                ...
                <div>
                    ...
                    <input type="number" name="minute" id="minute" value="{{ old('minute') }}" min="0" max="59" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none" placeholder="0-59" required>
