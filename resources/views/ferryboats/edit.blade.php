...
@section('title', 'Edit Ferry Boat')
@section('page-title', 'Edit Ferry Boat')
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-purple-600 to-purple-700">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h2 class="font-semibold text-white">Edit Ferry Boat</h2>
                    <p class="text-sm text-purple-100">Update the details below</p>
                </div>
            </div>
        </div>

        <form action="{{ route('ferryboats.update', $ferryboat) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="number" class="block text-sm font-medium text-slate-700 mb-2">Ferry Boat No. <span class="text-red-500">*</span></label>
                <input type="text" name="number" id="number" value="{{ old('number', $ferryboat->number) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none" placeholder="Enter boat number" required>
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $ferryboat->name) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none" placeholder="Enter boat name" required>
            ...
            <div>
                ...
                <select name="branch_id" id="branch_id" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none" required>
                    @foreach($branches as $branch)
                        <option value="{{ $branch->id }}" {{ old('branch_id', $ferryboat->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->branch_name }}</option>
                    @endforeach
                </select>
