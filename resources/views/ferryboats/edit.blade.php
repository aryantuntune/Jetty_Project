...
<div class="flex justify-center items-center min-h-screen bg-gray-200 container p-4">
    <div class="win-card w-1/2">
        ...
        <!-- Form -->
        <form action="{{ route('ferryboats.update', $ferryboat) }}" method="POST" class="p-4 space-y-3">
            @csrf
            @method('PUT')

            <div>
                <label for="number" class="block font-bold text-sm">Ferry Boat No.</label>
                <input type="text" name="number" id="number" value="{{ $ferryboat->number }}" class="form-input" required>
            </div>

            <div>
                <label for="name" class="block font-bold text-sm">Ferry Boat Name</label>
                <input type="text" name="name" id="name" value="{{ $ferryboat->name }}" class="form-input" required>
            </div>
                ...
                <select name="branch_id" id="branch_id" class="form-input" required>
                    @foreach($branches as $branch)
                        <option value="{{ $branch->id }}" 
                            {{ $branch->id == $ferryboat->branch_id ? 'selected' : '' }}>
                            {{ $branch->branch_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="win-footer mt-4">
                ...
                <button type="submit" class="btn-save">Update Boat</button>
            </div>
        </form>
    </div>
</div>
