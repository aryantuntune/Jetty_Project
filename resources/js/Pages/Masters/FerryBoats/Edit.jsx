...
export default function FerryBoatsEdit({ ferryboat, branches }) {
    const { data, setData, put, processing, errors } = useForm({
        number: ferryboat?.number || '',
        name: ferryboat?.name || '',
        branch_id: ferryboat?.branch_id || '',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('ferryboats.update', ferryboat.id));
    };
            ...
            <div className="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                        Update details for {ferryboat?.name}
                    </p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-slate-200 flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Modify the boat information</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="flex items-center justify-end space-x-3 pt-6 border-t border-slate-200">
                        ...
                        >
                            ...
                            <span>{processing ? 'Saving...' : 'Update Ferry Boat'}</span>
                        </button>
                    </div>
                </form>
            </div>
...
FerryBoatsEdit.layout = (page) => <Layout children={page} title="Edit Ferry Boat" />;
