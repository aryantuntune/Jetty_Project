...
export default function CheckersEdit({ checker, branches, ferryboats }) {
    ...
    const form = useForm({
        ...
        branch_id: checker.branch_id || '',
        ferry_boat_id: checker.ferry_boat_id || '',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        form.put(route('checker.update', checker.id));
    };

    return (
        <>
            <Head title={`Edit ${checker.name}`} />

            {/* Header */}
            <div className="mb-8">
                <Link
                    href={route('checker.index')}
                    className="inline-flex items-center space-x-2 text-slate-600 hover:text-slate-800 mb-4"
                >
                    <ArrowLeft className="w-4 h-4" />
                    <span>Back to Checkers</span>
                </Link>
                <h1 className="text-2xl font-bold text-slate-800">Edit Checker</h1>
                <p className="mt-1 text-sm text-slate-500">Update checker details</p>
            </div>
            ...
            <div className="max-w-2xl">
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                    <div className="px-6 py-4 border-b border-slate-200 flex items-center space-x-3">
                        ...
                        <div>
                            <h2 className="font-semibold text-slate-800">{checker.name}</h2>
...
CheckersEdit.layout = (page) => <Layout children={page} />;
