...
// Diagnostic Error Boundary — shows the EXACT error instead of a blank screen
class DiagnosticBoundary extends Component {
    ...
    componentDidCatch(error, errorInfo) {
        ...
        console.error('[DiagnosticBoundary]', error, errorInfo);
    }
    render() {
        if (this.state.hasError) {
            return (
                <div style={{ padding: 20, margin: 20, background: '#fef2f2', border: '2px solid #dc2626', borderRadius: 12, fontFamily: 'monospace' }}>
                    <h2 style={{ color: '#dc2626', margin: '0 0 10px' }}>⚠️ React Error Caught</h2>
                    <p style={{ color: '#991b1b', fontWeight: 'bold' }}>
                        {String(this.state.error?.message || this.state.error)}
                    </p>
                    <details style={{ marginTop: 10 }}>
                        <summary style={{ cursor: 'pointer', color: '#6b7280' }}>Stack Trace</summary>
                        <pre style={{ fontSize: 11, overflow: 'auto', maxHeight: 300, background: '#fff', padding: 10, borderRadius: 6 }}>
                    ...
                    <button onClick={() => this.setState({ hasError: false, error: null, errorInfo: null })}
                        style={{ marginTop: 12, padding: '8px 16px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer' }}>
                        Try Again
                    </button>
                </div>
            );
